...
def admin_interface2(parent,go_back_function):
    ...
    def save_donor():
        try:
            ...
            sql="""
            insert into save_donors (name,age,blood_group,city,contact,available)
            values(%s,%s,%s,%s,%s,%s)
            """
            data=(
                    name_entry.get(),
    ...
    ctk.CTkButton(footer, text="Clear",fg_color="gray", command=clear_form).pack(side="left", padx=10)
    ctk.CTkButton(footer,text="Back to Main Page",fg_color="gray",command=go_back).pack(side="left",padx=10)
